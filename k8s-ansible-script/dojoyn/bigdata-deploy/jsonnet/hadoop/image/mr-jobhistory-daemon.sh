...
case $startStop in
  ...
  (start)
    ...
    if [ "$nodaemon" = "nodaemon" ]; then
        start_pid=`ps aux | grep java | grep JobHistoryServer | awk '{print $2}'`
        if [ ! -z $start_pid ]; then
          echo $command running as process `echo $start_pid`.  Stop it first.
          exit 1
        fi
        echo starting $command
        cd "$HADOOP_MAPRED_HOME"
        nice -n $HADOOP_MAPRED_NICENESS "$HADOOP_MAPRED_HOME"/bin/mapred --config $HADOOP_CONF_DIR $command "$@"
    elif [ -f $pid ]; then
  ...
  (stop)
    if [ "$nodaemon" = "nodaemon" ]; then
      stop_pid=`ps aux | grep java | grep JobHistoryServer | awk '{print $2}'`
      if [ ! -z $stop_pid ]; then
        echo $stop_pid > $pid
      fi
    fi
